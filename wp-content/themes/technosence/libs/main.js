$(function () {
  ...
  jQuery(
    ".header__item__center li a, #leftside-navigation .nano-content li a"
  ).on("click", function (event) {
    event.preventDefault();

    var hrefSection = jQuery(jQuery(this).attr("href"));
    if (hrefSection.get(0))
      jQuery("html, body").animate(
        { scrollTop: hrefSection.offset().top + "px" },
        400
      );
  ...
  jQuery("body").bind("click", function () {
    setTimeout(function () {
      jQuery(".vipad").each(function () {
        if (!jQuery(this).data("clicked")) jQuery(this).removeClass("active");
      });
    }, 50);
  });
  ...
  jQuery(".cheks .config").on("click", function () {
    ...
    if (popup.get(0)) {
      ...
      if (phoneInput.get(0)) {
        phoneInput.attr(
          "data-invalid",
          phoneInput.attr("data-invalid").replace(/Viber|WhatsApp/, messenger)
        );
        phoneInput.attr(
          "placeholder",
          phoneInput.attr("placeholder").replace(/Viber|WhatsApp/, messenger)
        );
      }
    }
  });
...
function sliderDotsAnimation(slider) {
  ...
  slider
    .find(".k_svg_sl")
    .stop()
    .animate({ "stroke-dashoffset": 0 }, 4000, "swing", function () {
...
function serviceSlideChange(slider, direction) {
  ...
  if (direction == "prev") {
    ...
    if (!newActive.get(0))
      newActive = slider.find(".services_sl_block").first();
  } else {
    direction = parseInt(direction) || 0;
    newActive = slider
      .find(".services_sl_block[data-index=" + direction + "]")
      .first();
  }
...
function whySlideChange(index) {
  ...
  activeDot
    ...
    .animate({ "stroke-dashoffset": 0 }, 4000, "swing", function () {
      ...
      if (!jQuery(".why_slider_block[data-index=" + newIndex + "]").length)
        newIndex = 0;
...
function scrollFunction() {
  ...
  if ($(".company").length) {
    ...
    if (scrollTop >= offsetTop && !jQuery(".company").data("scrolled")) {
      jQuery(".polo_1").each(function (i, self) {
        ...
        jQuery(self).html(
          '<iframe src="/wp-content/themes/technosence/shape/' +
            id +
  ...
  links.each(function () {
    ...
    if (firstSection.get(0) || lastSection.get(0)) {
      ...
      if (top <= windowMiddle && scrollTop < top + height)
  ...
  if ($("#contacts").length) {
    ...
    if (
      scrollTop >= offsetTop &&
      !$("#google-map-container").data("scrolled")
    ) {
      jQuery.ajax({
        type: "GET",
...
jQuery(function () {
  ...
  jQuery(".popup form, .consultation__main form, .technical__main form").on(
    "submit",
    function (event) {
      event.preventDefault();
        ...
        jQuery.ajax({
          ...
          success: function (data) {
            if (data) {
              ...
              setTimeout(function () {
                if (formData.get("email_pdf"))
                  showPopup(null, jQuery(".pop_up_12").get(0), "pop_up_12");
...
function showPopup(
  ...
  if (_additionals)
    activePopup
      .find(".popup-item")
      .attr("data-additionals", JSON.stringify(_additionals));
...
function hidePopup() {
  var activProductsPopupClass = jQuery(".popup:not(.closed)").hasClass(
    "pop_up_fon_th3"
  );
